...
export default function ProductDetailPage() {
  ...
  const fetchProduct = async () => {
    ...
    try {
      console.log(`Fetching product ${id} from: http://localhost:3000/api/v1/products/${id}`);
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          throw new Error(`El producto con ID ${id} no existe en el servidor.`);
      ...
      // Get raw text first to debug (optional, remove in production)
      ...
      try {
        ...
        throw new Error(`El servidor devolvió un formato de datos inválido: ${parseError instanceof Error ? parseError.message : 'Error de parseo'}`);
      }
      ...
      if (error instanceof Error) {
        ...
        setError('No se pudieron cargar los detalles del producto. Verifica tu conexión o intenta más tarde.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const addToCart = () => {
    ...
    try {
      // Safely parse cart with error handling
      let cart = [];
      ...
      if (cartString && cartString.trim() !== '') {
        try {
          ...
          console.error('Failed to parse cart, resetting:', e);
          cart = [];
        }
      }
      
      const existing = cart.find((i: any) => i.id === product.id);
      const finalPriceValue = hasDiscount ? finalPrice : product.price;
      ...
      alert('Error al añadir el producto al carrito. Por favor, intenta nuevamente.');
    }
  };
    ...
    return (
      <div className="page-content" style={{ textAlign: 'center', padding: '4rem' }}>
        <div style={{ 
          ...
          <p className="text-crimson" style={{ marginBottom: '1.5rem', fontSize: '1.1rem' }}>
